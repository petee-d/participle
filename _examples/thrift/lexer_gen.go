...
func (l *lexerImpl) Next() (lexer.Token, error) {
	...
	switch state.name {
	case "Root":if match := matchNumber(l.s, l.p, l.states[len(l.states)-1].groups); match[1] != 0 {
			sym = -2
			groups = match[:]
		} else if match := matchIdent(l.s, l.p, l.states[len(l.states)-1].groups); match[1] != 0 {
			sym = -3
			groups = match[:]
		} else if match := matchString(l.s, l.p, l.states[len(l.states)-1].groups); match[1] != 0 {
			sym = -4
			groups = match[:]
		} else if match := matchWhitespace(l.s, l.p, l.states[len(l.states)-1].groups); match[1] != 0 {
			sym = -5
			groups = match[:]
		} else if match := matchPunct(l.s, l.p, l.states[len(l.states)-1].groups); match[1] != 0 {
			sym = -6
			groups = match[:]
		} else if match := matchComment(l.s, l.p, l.states[len(l.states)-1].groups); match[1] != 0 {
			sym = -7
			groups = match[:]
		}
	}
...
// [0-9]+
func matchNumber(s string, p int, backrefs []string) (groups [2]int) {
// [0-9] (CharClass)
...
// "[^"]*"
func matchString(s string, p int, backrefs []string) (groups [2]int) {
// " (Literal)
l0 := func(s string, p int) int {
...
// [\(-\),\.:<->\{\}]
func matchPunct(s string, p int, backrefs []string) (groups [2]int) {
// [\(-\),\.:<->\{\}] (CharClass)
l0 := func(s string, p int) int {
if len(s) <= p { return -1 }
rn := s[p]
...
// //(?-s:.)*
func matchComment(s string, p int, backrefs []string) (groups [2]int) {
// // (Literal)
l0 := func(s string, p int) int {
