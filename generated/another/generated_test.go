...
func TestGenerated(t *testing.T) {
	...
	var stuffOrig, stuffNew source.Stuff
	...
	assert.NoError(t, parser.ParseFromLexer(lexOrig, &stuffOrig))
	fmt.Printf("%+v\n", &stuffOrig)

	assert.NoError(t, ParseStuff(lexNew, &stuffNew, false))
	fmt.Printf("%+v\n", &stuffNew)

	assert.Equal(t, stuffOrig, stuffNew)
}
...
func BenchmarkNative(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		...
		lexCopy := lex.Clone()
		if err := source.StuffParser.ParseFromLexer(lexCopy, &stuff); err != nil {
